...
public class MISDataValidationHO {
	Map<String, String> ResultMap = new HashMap<String, String>();

	public MISDataValidationHO() {
		ResultMap.clear();
	}
	...
	public Map<String, String> FetchTargetMaster(Map DataMap) throws Exception {
		...
		String sql = "SELECT TARGET_CODE,to_char(START_DATE, 'DD-MON-YYYY') START_DATE,to_char(END_DATE, 'DD-MON-YYYY') END_DATE,TARGET_DESCRIPTION, ACTIVE_FLAG, TARGET_REMARKS,ACTIVATION	,	\r\n" + 
				"PARENT_CODE FROM target_master\r\n"
	...
	public Map<String, String> FetchTargetParentList(Map DataMap) throws Exception {
		...
		String sql = "select rtrim(xmlagg(xmlelement(e, target_code || ':' || target_description||'('||target_code||')', ',').extract('//text()') order by target_code)\r\n" + 
				...
				"     from (SELECT m.target_code,m.target_description FROM target_master m where m.active_flag='Y' and ACTIVATION='1' ) ORDER BY target_code ";
		Connection con = ob.GetConnection();
		ResultSet rs = null;
		PreparedStatement stmt = null;
		try {
	...
	public Map<String, String> addTargetDetails(Map DataMap) {
		...
		try {
			...
			cstmt.setString(6, DataMap.get("ParentTargetCode").toString());
			cstmt.setString(7, DataMap.get("TargetType").toString());			
			cstmt.setString(8, DataMap.get("Targetremarks").toString());
	...
	public Map<String, String> FetchSenctionAndDisburseData(Map DataMap) {
		ResultMap.put("ERROR_MSG", "");
		DBUtils ob = new DBUtils();
		String sql = "SELECT * FROM mis_loan_sanction m where m.branch_code=? and m.entry_date=?";
		Connection con = ob.GetConnection();
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
	...
	public Map<String, String> AddSanctionData(Map DataMap) {
		DBUtils ob = new DBUtils();
		Connection con = null;
		CallableStatement cstmt = null;
		ResultMap.put("ERROR_MSG", "");
		try {
			con = ob.GetConnection();
			cstmt = con.prepareCall("CALL pkg_mis_entry_ho.sp_loan_sanction(?,?,?,?,?,?,?)");
			cstmt.setString(1, DataMap.get("BranchCode").toString());
			cstmt.setString(2, DataMap.get("EntyDate").toString());
			cstmt.setString(3, DataMap.get("ZeroEquitySanction").toString());
			cstmt.setString(4, DataMap.get("OthersSanction").toString());
			cstmt.setString(5, "0");
			cstmt.setString(6, "0");
	...
	public Map<String, String> AddDisburseData(Map DataMap) {
		DBUtils ob = new DBUtils();
		Connection con = null;
		CallableStatement cstmt = null;
		ResultMap.put("ERROR_MSG", "");
		try {
			con = ob.GetConnection();
			cstmt = con.prepareCall("CALL pkg_mis_entry_ho.sp_loan_disburse(?,?,?,?,?,?)");
			cstmt.setString(1, DataMap.get("BranchCode").toString());
			cstmt.setString(2, DataMap.get("EntyDate").toString());
			cstmt.setString(3, DataMap.get("govtDisburse").toString());
			cstmt.setString(4, DataMap.get("ZeroEquityDisburse").toString());
			cstmt.setString(5, DataMap.get("OthersDisburse").toString());
	...
	public Map<String, String> AddLoanRecoveryData(Map DataMap) {
		...
		try {
			con = ob.GetConnection();
			cstmt = con.prepareCall("CALL pkg_mis_entry_ho.SP_LOAN_RECOVERY(?,?,?,?,?)");
			cstmt.setString(1, DataMap.get("BranchCode").toString());
			cstmt.setString(2, DataMap.get("EntyDate").toString());
	...
	public Map<String, String> AddCourtCaseSettlementData(Map DataMap) {
		...
		try {
			con = ob.GetConnection();
			cstmt = con.prepareCall("CALL pkg_mis_entry_ho.SP_CC_SETTLEMENT(?,?,?,?,?)");
			cstmt.setString(1, DataMap.get("BranchCode").toString());
			cstmt.setString(2, DataMap.get("EntyDate").toString());
	...
	public Map<String, String> AddKharidabariData(Map DataMap) {
		...
		try {
			con = ob.GetConnection();
			cstmt = con.prepareCall("CALL pkg_mis_entry_ho.SP_LOAN_KHARIDABARI(?,?,?,?,?)");
			cstmt.setString(1, DataMap.get("BranchCode").toString());
			cstmt.setString(2, DataMap.get("EntyDate").toString());
	...
	public Map<String, String> AddDisposalAuditObjection(Map DataMap) {
		...
		try {
			con = ob.GetConnection();
			cstmt = con.prepareCall("CALL pkg_mis_entry_ho.SP_LOAN_AUDIT_OBJECTION(?,?,?,?,?)");
			cstmt.setString(1, DataMap.get("BranchCode").toString());
			cstmt.setString(2, DataMap.get("EntyDate").toString());
	...
	public Map<String, String> AddFaultyLoanCaseData(Map DataMap) {
		...
		try {
			con = ob.GetConnection();
			cstmt = con.prepareCall("CALL pkg_mis_entry_ho.SP_DEED_RETURN_FL(?,?,?,?)");
			cstmt.setString(1, DataMap.get("BranchCode").toString());
			cstmt.setString(2, DataMap.get("EntyDate").toString());
