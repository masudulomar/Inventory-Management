...
public class FinanceOperation {
	...
	public Map<String, String> UpdateInitProfile(Map DataMap) {
		...
		try {
			String sql1 = "UPDATE PRMS_EMP_SAL E SET E.NEW_BASIC=?,E.PF_DEDUCTION_PCT=?,E.BANK_ACC=?,E.mod_by=?,mod_on=sysdate WHERE E.EMP_ID=?";
			...
			String sql2 = "UPDATE PRMS_EMP_SAL_HIST E SET E.NEW_BASIC=?, E.PF_DEDUCTION_PCT=?, E.BANK_ACC=?,E.mod_by=?,mod_on=sysdate WHERE E.EMP_ID=?"
			...
			stmt.setString(4, DataMap.get("User_Id").toString());
			stmt.setString(5, DataMap.get("EmployeeId").toString());
			stmt.setString(6, DataMap.get("EmployeeId").toString());
			stmt.executeUpdate();
	...
	public Map<String, String> UpdateAllowance(Map DataMap) {
		...
		try {
			String query = "UPDATE PRMS_ALLOWANCE" + "   SET TEL_ALLOWANCE       = ?,"
					...
					+ "       HILL_ALLWNC         = ?," + "       ARREAR              = ?,"
					+ "       REMARKS             = ?," + "		  HR_AREAR_ALW		  = ?, mod_by=?, mod_on=sysdate"
					+ " WHERE EMP_ID = ?";
			...
			stmt.setString(12, DataMap.get("User_Id").toString());			
			stmt.setString(13, DataMap.get("EmployeeId").toString());
			stmt.executeUpdate();
	...
	public Map<String, String> UpdateDeduction(Map DataMap) {
		...
		String sql = "UPDATE PRMS_DEDUC" + "   SET CAR_FARE            = ?," + "       CAR_USE             = ?,"
				...
				+ "       OTHER_DEDUC         = ?," + "       REMARKS             = ?" + ",HR_AREAR_DED=?, mod_by=?, mod_on=sysdate WHERE EMP_ID = ? ";
		Connection con = ob.GetConnection();
		PreparedStatement stmt = null;
		try {
			stmt = con.prepareStatement(sql);
			...
			stmt.setString(16, DataMap.get("User_Id").toString());
			stmt.setString(17, DataMap.get("EmployeeId").toString());
			stmt.executeUpdate();
			ResultMap.put("SUCCESS", "Data Sucessfully Updated");
			stmt.close();
		} catch (SQLException e) {
