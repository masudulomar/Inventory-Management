...
public class MISDataValidation {
	...
	public Map<String, String> AddDisburseData(Map DataMap) {
		...
		try {
			...
			cstmt = con.prepareCall("CALL PKG_MIS_ENTRY.sp_loan_disburse(?,?,?,?,?,?)");
			...
			if (error != null) {
			 ...
			 ResultMap.put("ERROR_MSG", " Error in AddDisburseData");
			e.printStackTrace();
		} finally {
			try {
				con.close();
	...
	public Map<String, String> TargetValueSetup(Map DataMap) {
		...
		try {
			...
			stmt.setString(14, DataMap.get("govtloan").toString());
			stmt.setString(15, DataMap.get("TargetCode").toString());
			stmt.setString(16, DataMap.get("BranchCode").toString());
			stmt.executeUpdate();
