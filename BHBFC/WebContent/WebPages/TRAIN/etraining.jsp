...
<script type="text/javascript">
	...
	function initValues() {		
		createTable();
		document.getElementById("EmpID").value="";  
		loadNameofInstituteList();
		loadNameofSubjectList();
		document.getElementById("EmpID").focus();
	}

	var arrHead = new Array();
	arrHead = [ 'Activity', 'PF No & Name','Subject of Training', 'Start Date', 'End Date','Training Institute' ];
     			    ...
     			    var obj = JSON.parse(responseMessage);
    	   			 if (obj.ERROR_MSG != "") {
    						alert(obj.ERROR_MSG);
    						 ...
    						 document.getElementById("SubjectOfTraining").focus();    						 
    					}   						    						
     		    });	
    	}		
    }
        ...
        for (var c = 0; c < arrHead.length; c++) {
            ...
            if (c == 0) {      // the first column.
                ...
                if(c==2){
                	 var ele = document.createElement('input');
                     ele.setAttribute('type', 'text');
              	      ele.setAttribute('value', document.getElementById("SubjectOfTraining").value);
              	      ele.setAttribute("readonly", true);
              	      td.appendChild(ele);
                }
                if (c==3){
                ...
                if (c==5){
                	 var ele = document.createElement('input');
                     ele.setAttribute('type', 'text');
                  	 ele.setAttribute('value', document.getElementById("NameofInstitute").value);
                  	 ele.setAttribute("readonly", true);
                  	 td.appendChild(ele);
                 }  
            }
    ...
    function AddToList(event){
    	 if(document.getElementById("SubjectOfTraining").value=="" 
 			||document.getElementById("StartDate").value==""
 			||document.getElementById("EndDate").value=="" 
 			||document.getElementById("NameofInstitute").value == ""){
 			alert("Please Fill up all the information !!");
 		}
 	else
 		{
 		 addTransaction();
     	// document.getElementById("SubjectOfTraining").value = ""; 
 	     document.getElementById("StartDate").value = "";
 	     document.getElementById("EndDate").value = ""; 
 	     document.getElementById("EmpID").focus();
 	    // document.getElementById("NameofInstitute").value = "";
    ...
    function submitTransaction() {
	  ...
	  if (c == true) {
      	...
      	SetValue("Class", "AdminOperation","N");
		SetValue("Method", "SaveTrainingData","L");	
      	xmlFinal();
    ...
    function loadNameofInstituteList(){
    	clear();	
    	SetValue("Class", "AccontingParameterSetup","N");
    	SetValue("Method", "GetInstituteList","L");
    	xmlFinal();
    	$.ajax({
    		  method: "POST",
    		  url: "TransactionServlet",
    		  ...
    		  .done(function( responseMessage ) {
    		    ...
    		    if (obj.ERROR_MSG != "") {
    				...
    				var item_srting=obj.INSTITUTE_LIST;	
    				var select = document.getElementById("NameofInstitute");                  
    ...
    function loadNameofSubjectList(){
    	clear();	
    	SetValue("Class", "AccontingParameterSetup","N");
    	SetValue("Method", "GetTrainingSubject","L");
    	xmlFinal();
    	$.ajax({
    		  method: "POST",
    		  url: "TransactionServlet",
    		  ...
    		  .done(function( responseMessage ) {
    		    ...
    		    if (obj.ERROR_MSG != "") {
    				...
    				var item_srting=obj.SUBJECT_LIST;	
    				var select = document.getElementById("SubjectOfTraining");                  
    				 var item_arrayList = item_srting.split('#');
    				 for(var i = 0; i < item_arrayList.length; i++) {
    					 item_arrayList[i] = item_arrayList[i].replace("/^\s*/", "").replace("/^\s*/", "");							 
    					    var item_keyValue = item_arrayList[i].split(':');
...
<body onload="initValues()">
 <center> <p>Training Information Entry</p></center>
	<fieldset>
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-15">
					<label for="SubjectOfTraining">Subject of Training</label>
				</div>

				<div class="col-20">
					<select id="SubjectOfTraining" name="SubjectOfTraining" style="width: 450px;">						
					</select>
				</div>					
			</div>
			...
			<div class="row">
				<div class="col-15">
					<label for="NameofInstitute">Training Institute</label>
				</div>
				
				<div class="col-20">
					<select id="NameofInstitute" name="NameofInstitute" style="width: 450px;">						
					</select>
				</div>	
